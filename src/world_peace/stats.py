...
class PortfolioStatistics:
    ...
    def sharp_ratio(self):
        ...
        portfolio_returns = [
            a.rate_of_return for a in self.portfolio.positions
        ] + historical_returns
        portfolio_std = stdev(portfolio_returns)
        sharp_ratio = (
            self.compound_annual_growth_rate() - self.risk_free_rate
        ) / portfolio_std
    ...
    def drawdown(self, *args, **kwargs) -> DrawDown:
        historical_portfolios: list[Portfolio] = [
            v for v in self.historical_portfolios.values()
        ]
        best_day: Portfolio = max(
            historical_portfolios, key=lambda p: p.total_market_value
        )
        worst_day: Portfolio = min(
            historical_portfolios[best_day.time_idx :],
            key=lambda p: p.total_market_value,
        )
        draw_down_percent = (
            best_day.total_market_value - worst_day.total_market_value
        ) / best_day.total_market_value
